...
class ColorDetector
{
    ...
    IplImage* color_detect(IplImage* Frame,int Flag) 
    {
        ...
        for(y=0;y<Frame->height;y++)
        {
    		...
    		{
    		    if((color[Flag].R_Min<ptr[3*x]) && (ptr[3*x]<color[Flag].R_Max) && (color[Flag].G_Min<ptr[3*x+1]) && (ptr[3*x+1]<color[Flag].G_Max) && (color[Flag].B_Min<ptr[3*x+2]) && (ptr[3*x+2]<color[Flag].B_Max))
    		        ptrp[3*x] = 255;
       		    else
    		        ptrp[3*x] = 0;
    		    ptrp++;
